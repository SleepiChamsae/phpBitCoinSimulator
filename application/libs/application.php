...
class application
{
    ...
    public function __construct()
    {
        ...
        $params['menu'] = isset($getParams[0]) && $getParams[0] != ''
                ?$getParams[0] : 'Board';
        $params['action'] = isset($getParams[1]) && $getParams[1] != ''
                ?$getParams[1] : 'index';
        $params['category'] = isset($getParams[2]) && $getParams[2] != ''
                ?$getParams[2] : null;
        $params['idx'] = isset($getParams[3]) && $getParams[3] != ''
                ?$getParams[3] : null;
        $params['pageNum'] = isset($getParams[4]) && $getParams[4] != ''
                ?$getParams[4] : null;
        
        if(!file_exists('application/controllers/'.$params['menu'].'Controller.php')) {
            echo "해당 컨트롤러가 존재하지 않습니다 :".$params['menu']."";
            exit();
        }
